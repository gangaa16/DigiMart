...
const loginUserCtrl = asyncHandler(async (req, res) => {
  ...
  if (findUser && (await findUser.isPasswordMatched(password))) {
    ...
    const token = jwt.sign(
      ...
      process.env.JWT_SECRET, // Use your actual JWT secret here
      { expiresIn: '1h' } // Example: token expires in 1 hour
    );

    // Set the token in a cookie or send it back in the response
    res.cookie("token", token, {
      httpOnly: true,
      maxAge: 3600000, // 1 hour in milliseconds
      // secure: true, // Uncomment if using HTTPS
      // sameSite: 'none', // Uncomment if using cross-origin requests
    });
...
const getMyOrders = asyncHandler(async(req,res)=>{
  try{
    const orders = await Order.find({user_id}).populate("user").populate("orderItems.product")
    res.jaon({
      orders
    })
